...
import { validateEmail, validatePassword, validateRequired, validateConfirmPassword } from '@/utils/validation';
import { registerUser, createUserProfile } from '@/services/auth.service';
...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    const errs = { fullName: nameErr ?? undefined, email: emailErr ?? undefined, password: pwErr ?? undefined, confirm: confirmErr ?? undefined };
    if (Object.values(errs).some(Boolean)) { setErrors(errs); return; }
    setErrors({});
    setLoading(true);
    try {
      const { uid } = await registerUser({ fullName: fullName.trim(), email: email.trim(), password });
      await createUserProfile(uid, { fullName: fullName.trim(), course: '', roleLabel: 'Student' });
      // Root layout will redirect to complete-profile
    } catch (err: any) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.form}>
            <FormInput label="Full name" value={fullName} onChangeText={setFullName} placeholder="Your full name" error={errors.fullName} />
            <FormInput label="Email" value={email} onChangeText={setEmail} placeholder="you@university.edu" keyboardType="email-address" autoCapitalize="none" error={errors.email} />
            <FormInput label="Password" value={password} onChangeText={setPassword} placeholder="At least 8 characters" secureTextEntry error={errors.password} />
            <FormInput label="Confirm password" value={confirm} onChangeText={setConfirm} placeholder="Repeat password" secureTextEntry error={errors.confirm} />
