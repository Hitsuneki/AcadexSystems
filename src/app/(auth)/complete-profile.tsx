...
export default function CompleteProfileScreen() {
  ...
  const handleContinue = async () => {
    ...
    if (courseErr) { setErrors({ course: courseErr }); return; }
    setErrors({});
    setLoading(true);
    try {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.form}>
            <FormInput label="Course / Program" value={course} onChangeText={setCourse} placeholder="e.g. BS Computer Science" error={errors.course} />
              ...
              <View style={styles.roleRow}>
                {ROLES.map((r) => (
                  <Pressable
                    ...
                    style={[styles.roleBtn, roleLabel === r && styles.roleBtnActive]}>
            ...
            <FormInput label="Bio (optional)" value={bio} onChangeText={setBio} placeholder="A short bio..." multiline numberOfLines={3} />

            <Pressable onPress={handleContinue} disabled={loading} style={[styles.primaryBtn, loading && styles.btnDisabled]}>
              {loading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.primaryBtnText}>Continue</Text>}
            </Pressable>
          </View>
        </ScrollView>
